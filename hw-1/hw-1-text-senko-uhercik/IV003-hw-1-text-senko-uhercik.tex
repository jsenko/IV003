...
Tieto opracie su znovy vykonatelne v linearnom case. Vysledne sucty urcia na ktorej particii sa algoritmus rekurzivne zavola, pripadne ak plati podmienka pre optimalny prvok postupnosti, vypocet skonci. To znamena ze parameter mozeme odstranit parameter $n$ ktory je po tejto uprave nepotrebny. Vysledny upraveny algoritmus je nasledovny:

\begin{algorithmic}[1]
    \Function{quick\_select\_mom}{$list$, $left$, $right$}
       	\State $pivot \gets$ \Call{median\_of\_medians\_index}{$list$, $left$, $right$}
        \State $pivot \gets$ \Call{partition}{$list$, $left$, $right$, $pivot$}
        \State $sumLeft \gets$ \Call{sum}{$list$, $left$, $pivot - 1$}
        \State $sumRight \gets$ \Call{sum}{$list$, $pivot + 1$, $right$}
        \If{$sumLeft \geq \frac{1}{2}$}
            \State \Return \Call{quick\_select\_mom}{$list$, $left$, $pivot - 1$}
        \ElsIf{$sumRight > \frac{1}{2}$}
            \State \Return \Call{quick\_select\_mom}{$list$, $pivot + 1$, $right$}
        \Else
            \State \Return $list[n]$
        \EndIf
    \EndFunction
