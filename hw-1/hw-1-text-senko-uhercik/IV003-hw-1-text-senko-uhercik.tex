...
\\
Quick Select \\
Vrati n-ty najmensi prvok pola, rekurzivne hlada v casti ohranicenej indexami left a right (vratane).
...
%        \EndIf
        ...
        \If{$n = pivotIndex$}
            \State \Return $list[n]$
        \ElsIf{$n < pivotIndex$}
            \State \Return \Call{quick\_select}{$list$, $left$, $pivotIndex - 1$, $n$}
        \Else
            \State \Return \Call{quick\_select}{$list$, $pivotIndex + 1$, $right$, $n$}
        \EndIf
    \EndFunction
\end{algorithmic}
...
\begin{algorithmic}[1]
    \Function{median\_of\_medians}{$list$}
        \State $groups \gets$ \Call{split\_into\_groups\_of}{$list$, $5$}
        \State $medians \gets$ \Call{new\_list}{}
        \ForAll{$group \in groups$}
            \State $median \gets$ \Call{quick\_median}{$group$}
...
Nevieme zlozitost funkcie quick\_select, preto skombinujeme algoritmy a spocitame algoritmu quick\_select\_MoM, ktory vznikne nahradenim  \\
