...
\begin{algorithmic}[1]
	...
	\State $isAce \gets$ \Call{is\_ace}{$maximum$, $maxX$, $maxY$}
	\If{$isAce$ }
		\State \Return $maximum$
	\Else
		\State $submatrix \gets$ \Call{make\_submatrix}{$matrix$,$startX$,$startY$,$endX$,$endY$}
		\State \Return \Call{search\_ace}{$submatrix$}
	\EndIf
...
\\
\\
    \Function{is\_ace}{$matrix$,$positionX$,$positionX$}
	\State $current \gets matrix[positionX][positionY]$
	\If{$current>matrix[positionX + 1 ][positionY] \And  \\
         		\pushcode[2] current>matrix[positionX - 1 ][positionY] \And
		current>matrix[positionX][positionY + 1] \And
		current>matrix[positionX][positionY - 1]$}
		\State \Return $true$
	\EndIf
